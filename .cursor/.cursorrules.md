...
프로젝트는 ESLint를 사용하여 순환 참조를 자동으로 감지합니다:
...
- `import/no-self-import`: 자기 자신을 import하는 경우 감지 (에러)
...
- [ ] `pnpm run lint` 실행 시 순환 참조 오류가 없는지 확인
...
// fileB.ts
import { functionA } from './fileA';  // 순환 참조!
...
export const functionA = () => { commonFunction(); };

// fileB.ts
import { commonFunction } from '../utils/common';
export const functionB = () => { commonFunction(); };
```
